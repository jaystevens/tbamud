...
ACMD(do_skillset) {
    ...
    if ((spell_info[skill].min_level[(pc)] >= LVL_IMMORT) && (pl < LVL_IMMORT)) {
        ...
        send_to_char(ch, "The minimum level for %s is %d for %ss.\r\n", spell_info[skill].name,
                     spell_info[skill].min_level[(pc)], pc_class_types[pc]);
    }

    /* find_skill_num() guarantees a valid spell_info[] index, or -1, and we
     * checked for the -1 above so we are safe here. */
    SET_SKILL(vict, skill, value);
    mudlog(BRF, LVL_IMMORT, TRUE, "%s changed %s's %s to %d.", GET_NAME(ch), GET_NAME(vict), spell_info[skill].name,
           value);
...
static int count_pages(char *str, struct char_data *ch) {
    ...
    for (pages = 1; (str = next_page(str, ch)); pages++); /* moved semi-colon to suppress warning */
    return (pages);
}
